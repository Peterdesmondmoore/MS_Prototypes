...
export default defineConfig({
  base: process.env.GITHUB_ACTIONS ? '/replace-with-repository-name/' : './',
  plugins: [react(), {
    ...
    generateBundle() {
      this.emitFile({ type: 'asset', fileName: 'mission-surface-deployment.json', source: JSON.stringify({
        ...
        pagesOrigin: process.env.MISSION_SURFACE_PAGES_ORIGIN ?? 'https://example.github.io/replace-with-repository-name',
